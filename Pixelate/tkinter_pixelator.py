...
class MainApplication(tk.Tk):
    ...
    def generate_image(image_path, percentage):
        global image_tk
        image_original=Image.open(image_path)
        new_dims=[int(round(dimension*(percentage/100))+1) for dimension in image_original.size]
        new_image=image_original.resize(new_dims).resize(image_original.size, resample=0)
        image_tk=ImageTk.PhotoImage(new_image)
        return new_image
    
    def generate_image_name(image, percentage):
        formatted_number = f"{percentage:21.17f}"
        formatted_number=formatted_number.strip().zfill(21)
        filename=re.search(r'([^\/]+)\.([a-zA-Z0-9]+)$',image).group(1)
        extension=re.search(r'([^\/]+)\.([a-zA-Z0-9]+)$',image).group(2)
        image_name=f"{filename}_{formatted_number}.{extension}"
        image_full_name=f"{MainApplication.destination}/{image_name}"
        print(f"Generated name for {image} at {percentage} percent is:\n {image_name}")
        return image_name, image_full_name

    def open_image_folder(folder=None):
        if not folder:
            folder=MainApplication.open_directory()
        files=os.listdir(folder)
        for file in files:
            MainApplication.open_image_file(file)

    def open_image_file(file=None):
        if not file:
            file=MainApplication.open_single_file()
        MainApplication.files.append(file)
        full_path=f"{MainApplication.source}/{file}"
        MainApplication.confirmed_images[file]={}
        MainApplication.confirmed_images[file]["Path"]=full_path
        MainApplication.confirmed_images[file]["Percentages"]=[100]
