...
The Elastic stack, created by Elastic, is an open-source collection of mainly three applications ([[Elasticsearch]], [[Logstash]], and [[Kibana]]) that work in harmony to offer users comprehensive search and visualization capabilities for real-time analysis and exploration of [[z]] sources.

![The Elastic Stack](BpLnfgDsc2WD%201.png)
...
![The Elastic Stack](8F2qNfHK5a84%201.png)
...
![Beats -> Logstash](jjJkwzDkh9h2%201.png)

`Beats` -\> `Elasticsearch` -\> `Kibana`

![Beats -> Elasticsearch](fhfUVuS9jZ8u%202.png)
...
![Beats -> Elasticsearch](VbhV3vC5AWX3%202.png)
...
The [[KQL]] query `event.code:4625` filters data in Kibana to show events that have the [[Windows event codes|]]. This Windows event code is associated with failed login attempts in a Windows operating system.
...
- Using KQL's free text search we can search for `"4625"`. In the returned records we notice `event.code:4625`, `winlog.event_id:4625`, and `@timestamp`
  ...
  - `@timestamp` typically contains the time extracted from the original event and it is [different from `event.created`](https://discuss.elastic.co/t/winlogbeat-timestamp-different-with-event-create-time/278160)![Free Text Search 1](9IVUWSP2NcHc%202.png)
- When it comes to disabled accounts, the aforementioned resource informs us that a SubStatus value of 0xC0000072 inside a 4625 Windows event log indicates that the account is currently disabled. Again using KQL's free text search we can search for `"0xC0000072"`. By expanding the returned record we notice `winlog.event_data.SubStatus` that is related to [Winlogbeat](https://www.elastic.co/guide/en/beats/winlogbeat/current/exported-fields-winlog.html)![Free Text Search 2](iWvqZTa2N95R%202.png)
...
![Use Case 1](Edz0ThbXfQ6p%202.png)
...
![Use Case Dev Lifecycle](YSQ3n267l1VQ%202.png)
...
![Use Case Dev Lifecycle](KGNbSuJE9fQb%202.png)
...
![Use Case Dev Lifecycle](zONJAAwdCxmM%202.png)
...
![Visualization 29](8BIabKERsUhP%202.png)
...
![New Dashboard](NmMmdf2eSJyY%201.png)

Now, to initiate the creation of our first visualization, we simply have to click on the "Create visualization" button.

![New Visualization](tqwcFiUILzXv%202.png)
...
![Visualization 1](2fcNIrWO7sTo%201.png)
...
![Visualization 2](FgoilA0U1WxN%202.png)
...
![Visualization 11](eW1gdgUVDsEW%202.png)
...
![Visualization 4](J77aX7tLFJ84%201.png)
...
![Visualization 5](qYU6UrN8ctec%201.png)

Let's configure the "Rows" settings as follows.

![Visualization 6](wZt5S4zjhD0t%201.png)
...
![Visualization 7](XRTmkYKQoN91%201.png)

In the "Metrics" window, let's select "count" as the desired metric.

![Visualization 8](FmWnQSK2wRC5%201.png)
...
![Visualization 9](UHK2KqAtxjP2%201.png)
...
![Visualization 12](ZmD1jtt3zgr5%201.png)
...
![Visualization 24](rL5Fc3GcuHC0.png)
...
![Visualization 16](V6fFMTuPCdWp.png)

Now, to initiate the creation of our first visualization, we simply have to click on the "Create visualization" button.

Upon initiating the creation of our first visualization, the following new window will appear with various options and settings.

![Visualization 1](9QfuWWig3e5g.png)
...
![Visualization 30](XH72r8NElCnq.png)
...
![Visualization 11](sGwVqyTeCtdR.png)

4. Lastly, this drop-down menu enables us to select the type of visualization we want to create. The default option displayed in the earlier image is "Bar vertical stacked". If we click on that button, it will reveal additional available options (image redacted as not all options fit on the screen). From this expanded list, we can choose the desired visualization type that best suits our requirements and data presentation needs.

![Visualization 4](lF28hLZ4ZS6U.png)
...
![Visualization 5](9NlUA9Wq8J58.png)

Let's configure the "Rows" settings as follows.

![Visualization 6](ONeDbgGbV7Re.png)

Moving forward, let's close the "Rows" window and proceed to enter the "Metrics" configuration.

![Visualization 7](jsGNhi3E6olI.png)

In the "Metrics" window, let's select "count" as the desired metric.

![Visualization 8](d9IL2Ghx7sSG.png)

One final addition to the table is to include another "Rows" setting to show the machine where the failed logon attempt occurred. To do this, we will select the `host.hostname.keyword` field, which represents the computer reporting the failed logon attempt. This will allow us to display the hostname or machine name alongside the count of failed logon attempts, as shown in the image.

![Visualization 31](GHgmKwUMoa5S.png)
...
![Visualization 16](4VZlr9bUF2lC.png)

Now, to initiate the creation of our first visualization, we simply have to click on the "Create visualization" button.

Upon initiating the creation of our first visualization, the following new window will appear with various options and settings.

![Visualization 1](CzFc48bzlmW3.png)
...
![Visualization 38](SN9DXA0YSyNB.png)

![Visualization 39](iOW4q4RsMQd8.png)

2. This field indicates the data set (index) that we are going to use. It is common for data from various infrastructure sources to be separated into different indices, such as network, Windows, Linux, etc. In this particular example, we will specify `windows*` in the "Index pattern".

3. This search bar provides us with the ability to double-check the existence of a specific field within our data set, serving as another way to ensure that we are looking at the correct data. We are interested in the `user.name.keyword` field. We can use the search bar to quickly perform a search and verify if this field is present and discovered within our selected data set. This allows us to confirm that we are accessing the desired field and working with accurate data.

![Visualization 11](JWsEdQQNVIru.png)

4. Lastly, this drop-down menu enables us to select the type of visualization we want to create. The default option displayed in the earlier image is "Bar vertical stacked". If we click on that button, it will reveal additional available options (image redacted as not all options fit on the screen). From this expanded list, we can choose the desired visualization type that best suits our requirements and data presentation needs.

![Visualization 4](dsYLDARHEe5g.png)
...
![Visualization 5](RFQKuuIBQ111.png)

Let's configure the "Rows" settings as follows.

![Visualization 6](rhtsNPHcD1LZ.png)

Moving forward, let's close the "Rows" window and proceed to enter the "Metrics" configuration.

![Visualization 7](eKlNxeCZGRBP.png)

In the "Metrics" window, let's select "count" as the desired metric.

![Visualization 8](LcOLQrUoGHen.png)
...
![Visualization 40](mjv3LyvfNknd.png)

![Visualization 41](dV2HvcAYeM7c.png)
...
![Visualization 43](OIrni4gJpu4h.png)
...
![Visualization 16](feAMyJTxCL0g.png)

Now, to initiate the creation of our first visualization, we simply have to click on the "Create visualization" button.

Upon initiating the creation of our first visualization, the following new window will appear with various options and settings.

![Visualization 1](yMZ0Ps1oxjHA.png)
...
![Visualization 44](a9Go38Vfwuk3.png)

2. This field indicates the data set (index) that we are going to use. It is common for data from various infrastructure sources to be separated into different indices, such as network, Windows, Linux, etc. In this particular example, we will specify `windows*` in the "Index pattern".

3. This search bar provides us with the ability to double-check the existence of a specific field within our data set, serving as another way to ensure that we are looking at the correct data. We are interested in the `user.name.keyword` field. We can use the search bar to quickly perform a search and verify if this field is present and discovered within our selected data set. This allows us to confirm that we are accessing the desired field and working with accurate data.

![Visualization 11](dacQvlFJYxLd.png)

4. Lastly, this drop-down menu enables us to select the type of visualization we want to create. The default option displayed in the earlier image is "Bar vertical stacked". If we click on that button, it will reveal additional available options (image redacted as not all options fit on the screen). From this expanded list, we can choose the desired visualization type that best suits our requirements and data presentation needs.

![Visualization 4](x8wQQmuaHYTo.png)
...
![Visualization 5](YjSqIDwF3JM1.png)

Let's configure the "Rows" settings as follows.

![Visualization 6](AuR9KTZ604oo.png)

Moving forward, let's close the "Rows" window and proceed to enter the "Metrics" configuration.

![Visualization 7](4Vh5V9oGvx2p.png)

In the "Metrics" window, let's select "count" as the desired metric.

![Visualization 8](GsPo1YJFmnSr.png)
...
![Visualization 46](9ld6qoZB2R6D.png)
...
![Visualization 47](3Z1yz6W67dpd.png)

![Visualization 48](tvrwH1dMglEh.png)

![Visualization 50](Xguo57rqF1Ya.png)
